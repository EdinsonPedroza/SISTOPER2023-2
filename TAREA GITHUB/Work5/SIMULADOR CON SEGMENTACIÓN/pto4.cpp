...
int main()
{
    Memory memory(1024, {256, 256, 256, 256}); // Crea una instancia de la estructura Memory con 1024 bytes de capacidad y cuatro segmentos de 256 bytes cada uno
    ...
    for (Process p : processes) { // Recorre los procesos del vector en orden inverso
        memory.freeMem(p.id); // Libera el segmento asignado al proceso
